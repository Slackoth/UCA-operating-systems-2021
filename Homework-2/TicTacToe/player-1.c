...
int main() {
    ...
    // Indicates player 1 has started 
    send.type = 1;
    send.matRow = 0;
    send.matCol = 0;
    send.messageType = 0;
    ...
    playerOneId = msgget(KEY, 0600 | IPC_CREAT);

    // Send message
    msgsnd(playerOneId, &send, messageSize, 0);

    // Wait until player 2 has started
    printf("Waiting for player 2...\n");

    // Recieve player 2 confirmation
    msgrcv(playerOneId, &recieve, messageSize, 2, 0);
    ...
    if(recieve.messageType != 0) {
        ...
        msgctl(playerOneId, IPC_RMID, 0);

        return 0;
    }

    // Show empty board to player
        showBoard(matrix);
    ...
    while (1) {
        ...
        if(checkBoundaries(row, col)) {
            if(isSquareTaken(row - 1, col - 1, matrix))
                ...
                takeSquare(row - 1, col - 1, matrix, 'X');

                showBoard(matrix);

                // Check if player one has won
                endGame = isThereAWinner(matrix, 'X');
				...
				printf("Sending move to player 2...\n");
                msgsnd(playerOneId, &send, messageSize, 0);

                printf("Waiting for player 2...\n");
				
                // Recieve other player's move
                printf("Recieving player 2's move...\n");
                msgrcv(playerOneId, &recieve, messageSize, 0, 0);

                // Reflect other player's move on board
                takeSquare(recieve.matRow - 1, recieve.matCol - 1, matrix, '0');

                showBoard(matrix);

                if(recieve.messageType == 3) {
